...
GLuint			g_surfaceWidth	= 320;
GLuint			g_surfaceHeight	= 240;
...
*/
			...
			g_mainSurface = eglCreateWindowSurface(g_display, g_config, hWnd, 0);
...
int enableGroupCounters(char *groupName, GLuint *monitor, GLuint *groupID)
{
    int         j;
    int          numGroups;
    GLuint       *groups;
    ...
    // enable the counters
    glSelectPerfMonitorCountersAMD(*monitor, GL_TRUE, *groupID, counters[i].numCounters, counters[i].counterList);
                 ...
                 GLuint *counterID)
{
    ...
    for ( j = 0; j < counters[i].numCounters; j++ )
    {
...
//    enableGroupCounters("MH", &monitor, &group[0]);
#if 0
    getCounterByName("MH", "AXI_READ_REQUESTS_ID_7", &group[0], &counter[0]);
    getCounterByName("MH", "AXI_READ_CHANNEL_BURSTS_ID_7", &group[1], &counter[1]);
...
void deinit_perf_counter()
{
    ...
    while ( (4 * wordCount) < bytesWritten )
    {
        ...
        glGetPerfMonitorCounterStringAMD(groupId, counterId, 256, NULL, curCounterName);
...
//	_CrtSetBreakAlloc(53);
    ...
    //  init performance counters for later retrieval
    init_perf_counter();
