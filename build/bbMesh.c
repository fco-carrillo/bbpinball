...
{
#if SHORT_INDEX
	int bytes = sizeof(GLushort) * count;
	int i;
#else
...
#if SHORT_INDEX
	for(i=0; i<count; i++)
		*(mesh->indices+i)=(GLushort)*(indices+i);
#else
	...
	if ( mesh->indexBuffer )
	{
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, bytes, mesh->indices, GL_STATIC_DRAW);
	}
...
#if SHORT_INDEX
		GLushort* index_base = mesh->indexBuffer ? 0 : mesh->indices;
#else
...
#if SHORT_INDEX
			glDrawElements(GL_TRIANGLES, subMesh->count, GL_UNSIGNED_SHORT, index_base + subMesh->start);
#else
